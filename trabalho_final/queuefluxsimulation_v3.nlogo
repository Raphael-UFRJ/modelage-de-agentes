...
to create-sector
  ;; Define os rótulos dos patches de forma mais eficiente
  foreach stages [
    stage-patch ->
...
to create-public
  crt 1000 [  ;; uso de crt em vez de create-students
    set breed students
    set shape "person"
    setxy -11 -16
...
to move-students
  ask students [
    ...
    ifelse current-index = (length stages - 1) [
      ...
      ifelse count students-on next-stage = 0 and time-in-stage >= random move-threshold [
        move-to next-stage
...
to display-average-times
  ...
  (foreach stage-names total-times total-counts [
    [stage-name total-time total-count] ->
      ...
      show (word "Average time in " stage-name ": " avg-time " (total time: " total-time ", total count: " total-count ")")
  ])
